...
void steger_warming_c_call_(const double* k1, const double* k2, const double* k3, const double* d, 
		const double* u , const double* v , const double* w , const double* sv, double* fp, double* fn, int *pstep){
	...
	*(fp+0*step) = (*d) / gama2 * (gama2_2 * lamb[0]      + lamb[1]     + lamb[2]     );
	*(fp+1*step) = (*d) / gama2 * (gama2_2 * lamb[0]*(*u) + lamb[1]*usv + lamb[2]*usv_);
	*(fp+2*step) = (*d) / gama2 * (gama2_2 * lamb[0]*(*v) + lamb[1]*vsv + lamb[2]*vsv_);
	*(fp+3*step) = (*d) / gama2 * (gama2_2 * lamb[0]*(*w) + lamb[1]*wsv + lamb[2]*wsv_);
	*(fp+4*step) = (*d) / gama2 * (gama_1  * lamb[0]*((*u)*(*u) + (*v)*(*v) + (*w)*(*w))+
			0.5   * lamb[1]*( usv*usv  +  vsv*vsv  +  wsv*wsv )+
			0.5   * lamb[2]*(usv_*usv_ + vsv_*vsv_ + wsv_*wsv_)+
			gama_w  * (lamb[1] + lamb[2]) * (*sv)*(*sv)          );
	for(int i=0; i<3; i++)
		lamb[i]=0.5*(lambda[i]-sqrt(lambda[i]*lambda[i]+epsilon_2));
	*(fn+0*step) =(*d) / gama2 * (gama2_2 * lamb[0]      + lamb[1]     + lamb[2]     );
	*(fn+1*step) =(*d) / gama2 * (gama2_2 * lamb[0]*(*u) + lamb[1]*usv + lamb[2]*usv_);
	*(fn+2*step) =(*d) / gama2 * (gama2_2 * lamb[0]*(*v) + lamb[1]*vsv + lamb[2]*vsv_);
	*(fn+3*step) =(*d) / gama2 * (gama2_2 * lamb[0]*(*w) + lamb[1]*wsv + lamb[2]*wsv_);
	*(fn+4*step) =(*d) / gama2 * (gama_1  * lamb[0]*((*u)*(*u) + (*v)*(*v) + (*w)*(*w))+
			0.5   * lamb[1]*( usv*usv  +  vsv*vsv  +  wsv*wsv )+
			0.5   * lamb[2]*(usv_*usv_ + vsv_*vsv_ + wsv_*wsv_)+
			gama_w  * (lamb[1] + lamb[2]) * (*sv)*(*sv)          );
}
...
void dfdx_p_c_(const int *case_lb, const int *case_ub, const int *jj, const int *nn, const double *dx, double fp[], int *pstep){
	int i;
	const int BE = 6;
...
void dfdx_p_c_v1_(const int *case_lb, const int *case_ub, const int *jj, const int *nn, const double *dx, double fp[], double fpx[]){
	int i,m;
	const int BE_1 = 5;
	const int BE = 6;
	const int nn0 = *nn;
