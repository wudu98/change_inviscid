...
subroutine steger_warming(k1,k2,k3,d,u,v,w,sv,Fp,Fn)
    ...
    call steger_warming_c(k1, k2, k3, d,u,v,w,sv, Fp_c, Fn_c)

     error = 0
     do i=1,5
        error = error + DABS(Fp_c(i)-Fp(i))
        error = error + DABS(Fn_c(i)-Fn(i))
     end do

     if(DABS(error) > 1e-4) then
        write(*,*) "Error steger_warming", Fp(1), Fp_c(1)
     endif 
  return
end
!**********************************************************************************************************************************
...
do m=1,5
  ...
  return
   ...
   call fh_p_weno3_c(f_1, f0, f1, fhp_c);

  if(DABS(fhp) > 1e-6 .AND. DABS(fhp - fhp_c)/fhp > 1e-4) then
      write(*,*) "Error fh_p_weno3", fhp, fhp_c
  endif 
  return
 end
...
subroutine fh_p_weno5(f_2,f_1,f0,f1,f2,fhp)   !positively biased
  implicit none
  integer ::  i
  real*8  ::  f_2,f_1,f0,f1,f2,fhp ,fhp_c 
  ...
  call fh_p_weno5_c(f_2, f_1, f0, f1, f2, fhp_c);

  if(DABS(fhp) > 1e-6 .AND. DABS(fhp - fhp_c)/fhp > 1e-4) then
      write(*,*) "Error fh_p_weno5", fhp, fhp_c
  endif
 return
end
...
subroutine fh_p_weno7(f_3,f_2,f_1,f0,f1,f2,f3,fhp)   !positively biased
  implicit none
  integer ::  i
  real*8  ::  f_3,f_2,f_1,f0,f1,f2,f3,fhp  , fhp_c 
...
!optimal weight
  ...
  call fh_p_weno7_c(f_3, f_2, f_1, f0, f1, f2, f3, fhp_c);

  if(DABS(fhp) > 1e-6 .AND. DABS(fhp - fhp_c)/fhp > 1e-4) then
      write(*,*) "Error fh_p_weno7", fhp, fhp_c
  endif
 return
end
